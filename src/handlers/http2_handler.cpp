...
std::vector<HTTP2Frame> HTTP2Response::to_frames(uint32_t max_frame_size) const {
    ...
    // Build headers frame
    HTTP2Frame headers_frame = HTTP2Parser::build_headers_frame(*this, stream_id, true, body.empty());
...
bool HTTP2Parser::parse_connection_preface(const std::vector<uint8_t>& data, size_t& offset) {
    ...
    std::string preface(data.begin() + offset, data.begin() + offset + HTTP2_CONNECTION_PREFACE_LEN);
...
HTTP2Parser::ParseResult HTTP2Parser::parse_frame(const std::vector<uint8_t>& data,
                                                   size_t& offset,
                                                   HTTP2Frame& frame) {
...
HTTP2Frame HTTP2Parser::build_headers_frame(const HTTP2Request& request,
                                             uint32_t stream_id,
                                             bool end_headers,
                                             bool end_stream) {
    HTTP2Frame frame;
    ...
    frame.flags = 0;
    if (end_headers) frame.flags |= HTTP2Flags::END_HEADERS;
    if (end_stream) frame.flags |= HTTP2Flags::END_STREAM;
...
HTTP2Frame HTTP2Parser::build_headers_frame(const HTTP2Response& response,
                                             uint32_t stream_id,
                                             bool end_headers,
                                             bool end_stream) {
    HTTP2Frame frame;
...
HTTP2Frame HTTP2Parser::build_data_frame(uint32_t stream_id,
                                         const std::vector<uint8_t>& data,
                                         bool end_stream) {
    HTTP2Frame frame;
...
HTTP2Frame HTTP2Parser::build_window_update_frame(uint32_t stream_id, uint32_t window_size_increment) {
    HTTP2Frame frame;
...
HTTP2Connection::HTTP2Connection()
    : next_stream_id_(1)
    , client_preface_received_(false) {
}
