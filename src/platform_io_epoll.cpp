...
public:
    ...
    bool add_fd(int fd, uint32_t events, void* user_data) override {
        ...
        if (events & EVENT_ERROR) ev.events |= EPOLLERR | EPOLLHUP;
        if (events & EVENT_EDGE_TRIGGER) ev.events |= EPOLLET;
    ...
    int wait(std::vector<Event>& events, int timeout_ms) override {
        ...
        for (int i = 0; i < nfds; ++i) {
            ...
            if (epoll_events[i].events & EPOLLIN) ev.events |= EVENT_READ;
            if (epoll_events[i].events & EPOLLOUT) ev.events |= EVENT_WRITE;
            if (epoll_events[i].events & (EPOLLERR | EPOLLHUP)) ev.events |= EVENT_ERROR;
