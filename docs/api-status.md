...
| example/http2 | Multiplexing | **Stable** | Each fully-arrived request dispatches as a detached coroutine via `IOContext::schedule`, so the frame-reading loop keeps making progress while handlers run. All wire writes funnel through a single per-connection writer coroutine for per-frame atomicity, but frames across streams are free to interleave. Test: `HTTP2MultiplexingTest.FrameLoopUnblockedBySlowHandler` proves the frame loop emits PING-ACK while a handler is mid-await. |
