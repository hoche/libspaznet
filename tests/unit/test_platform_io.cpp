...
TEST_F(PlatformIOTest, ModifyFd) {
    ...
    EXPECT_TRUE(platform_io->modify_fd(fd, PlatformIO::EVENT_READ | PlatformIO::EVENT_WRITE, new_data));
...
TEST_F(PlatformIOTest, WaitTimeout) {
    ...
    EXPECT_GE(elapsed.count(), 90);  // Should wait at least 90ms
    EXPECT_EQ(num_events, 0);  // No events
