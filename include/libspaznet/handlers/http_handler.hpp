...
struct HTTPRequest {
    ...
    std::string request_target;      // Request target (origin-form, absolute-form, etc.) per RFC 9112 Section 3.2
    std::string version;             // HTTP version (e.g., "1.1") per RFC 9112 Section 2.5
    std::unordered_map<std::string, std::string> headers;  // Header fields (field-name: field-value) per RFC 9112 Section 5.5
...
public:
    ...
    static ParseResult parse_request(const std::vector<uint8_t>& buffer, 
                                      HTTPRequest& request,
                                      size_t& bytes_consumed);
    
    // Parse HTTP response from buffer
    static ParseResult parse_response(const std::vector<uint8_t>& buffer,
