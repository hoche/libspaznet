...
public:
    ...
    static ParseResult parse_headers_frame(const HTTP2Frame& frame,
    ...
    static HTTP2Frame build_headers_frame(const HTTP2Request& request,
                                          uint32_t stream_id,
                                          bool end_headers = true,
                                          bool end_stream = false);
    
    // Build HEADERS frame from response
    static HTTP2Frame build_headers_frame(const HTTP2Response& response,
                                          uint32_t stream_id,
                                          bool end_headers = true,
                                          bool end_stream = false);
    
    // Build DATA frame
    static HTTP2Frame build_data_frame(uint32_t stream_id,
                                       const std::vector<uint8_t>& data,
                                       bool end_stream = false);
...
public:
    ...
    const HTTP2Settings& get_settings() const { return settings_; }
